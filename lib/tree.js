...
class Tree {
    ...
    // Time Complexity: ?
    // Space Complexity: ?
    heightHelper(currentNode, num) {
        if (currentNode === null) return 0;

        if (currentNode.left === null && currentNode.right === null) return num;

        if (currentNode.right !== null && currentNode.left !== null) {
            return Math.max(this.heightHelper(currentNode.left, num + 1), this.heightHelper(currentNode.right, num + 1));
        } else if (currentNode.right !== null) {
            return this.heightHelper(currentNode.right, num + 1);
        } else {
            return this.heightHelper(currentNode.left, num + 1);
        }
    }
